<?php
/**
 * Menu order for custom post types
 *
 * @since    1.2
 */
...
add_action('admin_enqueue_scripts', function($hook) {
    if ($hook !== 'edit.php' || get_post_type() !== 'course') return;
    ?>
    <script>
    // Vanilla JS drag-and-drop for course ordering
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateOrder() {
            ...
            fetch(ajaxurl, {
                ...
                body: 'action=course_menuorder&nonce=' + encodeURIComponent('<?php echo wp_create_nonce('course_menuorder'); ?>') + '&ids=' + encodeURIComponent(JSON.stringify(ids))
...
add_action('wp_ajax_course_menuorder', function() {
    if (!current_user_can('edit_others_posts') || !isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'course_menuorder')) {
...
// 4. Order course posts by menu_order by default in admin
add_action('pre_get_posts', function($query) {
    if (is_admin() && $query->is_main_query() && $query->get('post_type') === 'course' && !$query->get('orderby')) {
