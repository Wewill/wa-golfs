...
wp.domReady(() => {
  ...
  const featuredObserver = new MutationObserver((mutations, obs) => {
    ...
    if (featuredPanel) {
      ...
      addCustomText(
        featuredPanel,
        '<p class="post-description"><span class="label">INFO</span> This image will be used as a logotype</p><p><span class="important">Provide at least 1000x1000px image in transparent * .png format.</span></p>'
      );
  ...
  const taxonomiesObserver = new MutationObserver((mutations, obs) => {
    const taxonomiesPanel = document.querySelector(
    ...
    if (taxonomiesPanel) {
      ...
      addCustomText(
        taxonomiesPanel,
        '<p class="post-description"><span class="label">INFO</span> Both choose a geography & geolocation of attached farm. </p><p><span class="important">Only one geography is allowed.</span></p>'
      );
      obs.disconnect(); // Stop observing once we've added our custom text
    }
  });
...
// Display a notice if no featured image set for custom post types
(function (wp) {
  var __ = wp.i18n.__;
  ...
  function MyFeaturedImageNotice() {
    ...
    useEffect(() => {
      ...
      const customPostTypes = ["competitions", "course"];

      // Check if the post type is one of the custom post types
      if (customPostTypes.includes(postType)) {
    ...
    }, [postType, featuredImageId]); // Re-run this effect only if postType or featuredImageId changes.
